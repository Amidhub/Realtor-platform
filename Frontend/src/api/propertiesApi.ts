...
type BackendCreatePropertyResponse =
  | number
  | {
      id?: number
      listing_id?: number
...
function extractCreatedPropertyId(data: BackendCreatePropertyResponse) {
  if (typeof data === 'number') {
    return data
  }

  const possibleId =
    data.id ?? data.listing_id ?? data.listing?.id ?? data.data?.id ?? data.data?.listing_id
  ...
  throw new Error(`Backend did not return listing id: ${JSON.stringify(data)}`)
}
...
export async function createProperty(data: CreatePropertyRequest) {
  const requestData: CreatePropertyRequest = {
    ...
    // Backend не принимает investment: null, поэтому отправляем пустой объект.
  ...
  const response = await apiClient.post<BackendCreatePropertyResponse>(
    '/listings/add_listing',
    requestData,
  )

  const id = extractCreatedPropertyId(response.data)
...
export async function updateProperty(
  ...
  const requestData: Partial<CreatePropertyRequest> = {
    ...data,
    investment:
